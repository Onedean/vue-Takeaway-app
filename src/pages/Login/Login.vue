        ...
        <form>
          <div :class="{on:loginWay}">
            <section class="login_message">
              ...
              <button :disabled="!getColor" class="get_verification" :class="{right_phone:getColor}" @click.prevent="getCode">{{computeTime>0?`已发送(${computeTime}s)`:'获取验证码'}}</button>
            </section>
            <section class="login_verification">
              <input type="tel" maxlength="8" placeholder="验证码" v-model="code">
            </section>
            <section class="login_hint">
              温馨提示：未注册小陈外卖帐号的手机号，登录时将自动注册，且代表已同意
    ...
    methods:{
      ...
      async login(){
        ...
        if(this.loginWay){  // 短信登录验证
          const {getColor,phone,code} = this
          if(!getColor){
        ...
        .login_header_title
          ...
          >a
            ...
            &.on
              color #38B8FF
              font-weight 700
              border-bottom 2px solid #38B8FF
      .login_content
        >form
          >div
            display none
