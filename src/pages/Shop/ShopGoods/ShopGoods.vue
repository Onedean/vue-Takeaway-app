        ...
        <ul>
          <li class="menu-item" v-for="(good,index) in goods" :key="index" :class="{current:index===currentIndex}" @click="clickMenuItem(index)">
            <span class="text  bottom-border-1px">
              <img class="icon" :src="good.icon" alt="icon" v-if="good.icon">
              {{good.name}}
            </span>
          </li>
        </ul>
...
<script>
  ...
  export default {
    ...
    data(){
      return {
        scrollY:0,  // 右侧滑动的Y轴坐标
        tops:[]  // 所有右侧分类li的top组成的数组（列表第一次显示后就不变化）
      }
    },
    mounted(){
      this.$store.dispatch('getGoods',() => {  // 数据更新后执行
    ...
    computed:{
      ...mapState(['goods']),
      // 计算当前分类的下标
      currentIndex(){ // 初始和相关数据发生改变时调用
        // 得到条件数据
        const {scrollY,tops} = this
        // 根据条件计算产生结果
        const index = tops.findIndex((top,index) => {
          // 当前top <= scrollY < 下一个top
    ...
    methods:{
      ...
      _initScroll(){
        ...
        // 给右侧列表绑定scrollEnd监听，解决惯性滑动右侧列表不改变bug
